...
本文档面向在 Arch Linux 上从源码构建 NyanChat Release APK 的开发者。
...
| Gradle 9.5.0 | 阿里云镜像，项目已配置 | 132 MB | 291 MB | 运行 `./gradlew` 时自动下载 |
...
> **注意**：这里必须使用 `android-37.0`，而不是 `android-37`。`workspace` 模块包含 C/C++ 源码，因此 NDK 和 CMake 不能省略。
...
首次构建会自动下载 Gradle 9.5.0 和全部 Android/Kotlin/Compose 依赖，耗时较长。
