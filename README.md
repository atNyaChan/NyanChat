...
> [!IMPORTANT]
> 本项目正在快速开发中，本文档中的很多功能尚未实装，且代码未经充分测试，目前请勿使用该项目。

## ✨ 在 RikkaHub 上进行的改进：（为什么用 NyanChat）

- 🧠 **添加了实验性的[潮汐记忆](https://github.com/0xblewalker/tidal-memory)支持** — 来自 [@3nvoy](https://www.xiaohongshu.com/user/profile/67f3dfbe000000000e0113e5) 老师的开源项目！让 AI 的记忆更接近真人，并优化缓存命中率。
- 📍 **设备状态感知** — 可获取位置、电量、晃动等信息，让 AI 更接近现实
...
- 🧹 **重构了部分设置页面和 UI 布局** — 更清晰的布局，更容易上手

> 完整修改详见 [NyanwChanges.md](docs/NyanwChanges.md)
...
- 支持从 RikkaHub 2.x 导入数据，也可以导出数据到 RikkaHub Latest（**聊天记录可以双向无损迁移！**）。
- 注意：
  - 由于 RikkaHub 限制，部分数据（如工作区文件、助手头像和背景）无法从 RikkaHub 导入。
  - 导出到 RikkaHub 时，NyanChat 独有的设置和元数据（如模型价格、潮汐记忆详情）会丢失。
...
请使用 [Releases](https://github.com/atNyaChan/NyanChat/releases/latest) 中的版本，或 [自行编译应用](docs/BuildArchLinuxZh.md)。
